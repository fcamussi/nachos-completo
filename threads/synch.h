...
{
  public:
    Port(const char* debugName, int port);
    ~Port();
    const char* getName() { return (name); }
  ...
  private:
    ...
    int readyToSend; // bandera de listo para enviar
    int readyToReceive; // bandera de listo para recibir
    int sent; // bandera de enviado
    int received; // bandera de recibido
    int *buffer; // buffer donde almaceno el buffer del receptor
    Lock *lock; // lock asociado a las variables de condición
    Lock *busyToSend; // lock para evitar que se envíe 2 veces simultaneamente
    Lock *busyToReceive; // lock para evitar que se reciba 2 veces simultaneamente
    Condition *send; // variable de condición asociado al emisor
    Condition *receive; // variable de condición asociado al receptor
};
...
{
  public:
    Message(const char* debugName, int nPorts);
    ~Message();
    const char* getName() { return (name); }
    void Send(int port, int menssage);
    void Receive(int port, int *message);
  private:
    const char *name;
    int nPorts; // cantidad de puertos
    Port **ports; // array para almacenar los punteros a Port
    Lock *lock; // lock para evitar que se aloje 2 veces el mismo puerto
