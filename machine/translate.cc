...
// from the virtual address
    ...
    *physAddr = pageFrame * PageSize + offset;
    ...
    // Almaceno las páginas virtuales referenciadas
    // Si hay más de una referencia seguida a la misma página
    // me quedo con una sola
    // (borrar fichero a mano si existe, antes de generarlo)
    /*static int prevVirtPage = -1;
    int virtPage = virtAddr / PageSize;
    if (prevVirtPage != virtPage)
    {
        FILE *file = fopen("PAGE.refs", "a");
        fprintf(file, "%010d\n", virtPage);
        fclose(file);
        prevVirtPage = virtPage;
    }*/
